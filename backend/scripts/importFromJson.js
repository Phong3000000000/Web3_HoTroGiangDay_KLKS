...
function safeRequire(modulePath, label) {
  try {
    ...
    console.warn(`Skipping model ${label}: ${error.message}`);
    return null;
  }
}

// Import available models only
const DanhMucPhongBan = safeRequire('../models/DanhMucPhongBan', 'DanhMucPhongBan');
const HoSoNhanVien = safeRequire('../models/HoSoNhanVien', 'HoSoNhanVien');
const ChamCong = safeRequire('../models/ChamCong', 'ChamCong');
const DanhGiaKpi = safeRequire('../models/DanhGiaKpi', 'DanhGiaKpi');
const PhanHoiKhachHang = safeRequire('../models/PhanHoiKhachHang', 'PhanHoiKhachHang');
const XepHangNhanVien = safeRequire('../models/XepHangNhanVien', 'XepHangNhanVien');
const LuongThuong = safeRequire('../models/LuongThuong', 'LuongThuong');
const SmartContractLogs = safeRequire('../models/SmartContractLogs', 'SmartContractLogs');
const AuditLogs = safeRequire('../models/AuditLogs', 'AuditLogs');
const QrAuthentication = safeRequire('../models/QrAuthentication', 'QrAuthentication');
const AiModelMetadata = safeRequire('../models/AiModelMetadata', 'AiModelMetadata');
const EventLogsUser = safeRequire('../models/EventLogsUser', 'EventLogsUser');
